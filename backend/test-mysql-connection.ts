...
import { getMySQL, closeMySQL } from "./database/database";
...
async function testBunSQLConnection() {
    try {
        ...
        await db.begin(async (tx) => {
            await tx`INSERT INTO kv_store (type, name, json_data) VALUES (${'test'}, ${'transaction_test'}, ${JSON.stringify({test: 'data'})})`;
            await tx`UPDATE kv_store SET json_data = ${JSON.stringify({test: 'updated'})} WHERE type = ${'test'} AND name = ${'transaction_test'}`;
        });
        console.log('✅ 事务测试成功');
